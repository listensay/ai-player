...
mod tests {
    ...
    fn post(db: &mut Connection, endpoint: &str, value: Value) {
        request(db, endpoint, "POST", json!({}), value).unwrap();
    }
    fn get(db: &mut Connection, endpoint: &str, query: Value) -> Value {
        request(db, endpoint, "GET", query, Value::Null).unwrap()
    }
    #[test]
    fn preserves_all_learning_records_and_setting_types() {
        let mut db = setup();
        post(
            &mut db,
            "recent-courses",
            json!({"id":"c1","name":"中文课程","videoCount":2,"lastVideoPath":"基础/01.mp4"}),
        );
        post(
            &mut db,
            "progress",
            json!({"courseId":"c1","path":"基础/01.mp4","time":24,"duration":120,"ratio":0.2,"done":false}),
        );
        post(
            &mut db,
            "notes",
            json!({"courseId":"c1","videoPath":"基础/01.mp4","content":"笔记 [00:24]"}),
        );
        post(
            &mut db,
            "guide",
            json!({"courseId":"c1","plan":{"summary":"路线"},"metadata":{},"includeOptional":true,"mastery":{},"questions":[],"today":null}),
        );
        post(
            &mut db,
            "practice",
            json!({"courseId":"c1","videoPath":"基础/01.mp4","records":[{"id":"p1"}]}),
        );
        post(
            &mut db,
            "check-in",
            json!({"courseId":"c1","days":[{"date":"2026-09-23","seconds":180,"targetSeconds":600,"checkedAt":null}]}),
        );
        post(
            &mut db,
            "note-images",
            json!({"courseId":"c1","videoPath":"基础/01.mp4","name":"图.png","dataBase64":"data:image/png;base64,AA=="}),
        );
        assert_eq!(
            get(&mut db, "recent-courses", json!({"id":"c1"}))["name"],
            "中文课程"
        );
        assert_eq!(
            get(&mut db, "progress", json!({}))["c1"]["基础/01.mp4"]["time"].as_f64(),
            Some(24.0)
        );
        assert_eq!(
            get(
                &mut db,
                "notes",
                json!({"courseId":"c1","videoPath":"基础/01.mp4"})
            )["content"],
            "笔记 [00:24]"
        );
        assert_eq!(
            get(&mut db, "guide", json!({"courseId":"c1"}))["includeOptional"],
            true
        );
        assert_eq!(
            get(&mut db, "practice", json!({"courseId":"c1"}))["基础/01.mp4"][0]["id"],
            "p1"
        );
        assert_eq!(
            get(&mut db, "check-in", json!({"courseId":"c1"}))["2026-09-23"]["seconds"].as_f64(),
            Some(180.0)
        );
        assert_eq!(
            get(
        ...
        for value in [
            ...
            json!(2),
            json!(false),
            json!({"profiles":[{"model":"test"}]}),
            Value::Null,
        ] {
            post(&mut db, "settings", json!({"key":"test","value":value}));
            assert_eq!(get(&mut db, "settings", json!({"key":"test"})), value);
        }
    }
    #[test]
    fn invalid_batch_rolls_back() {
        let mut db = setup();
        assert!(request(&mut db,"check-in","POST",json!({}),json!({"courseId":"c1","days":[{"date":"2026-09-23","seconds":10,"targetSeconds":600},{}]})).is_err());
        assert_eq!(
            get(&mut db, "check-in", json!({"courseId":"c1"})),
            json!({})
        );
    }
}
