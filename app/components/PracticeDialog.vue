...
const labels = { explain: '概念解释', code: '代码练习', task: '实践任务' }
      ...
      <div class="scroll-soft min-h-0 flex-1 space-y-5 overflow-y-auto p-5 sm:p-7">
        ...
        <details class="rounded-2xl border border-linen bg-pure-white p-4" :open="!current">
          ...
          <div class="mt-3 space-y-3">
            <p class="text-caption leading-relaxed text-stone">生成练习时，将选取的字幕、本课笔记文字和补充材料发送至所配置的 AI 服务；提交作答时另发送题目和答案。最多选取 12000 字，不发送视频和图片。笔记含当前尚未保存的文字。</p>
        ...
        <section v-if="current" :key="current.id" class="space-y-4 rounded-2xl border border-linen bg-pure-white p-4 sm:p-5" aria-label="当前练习">
          ...
          <label class="block text-body-sm font-bold">作答内容
            <textarea :value="current.draft" aria-label="作答内容" maxlength="8000" rows="6" :disabled="!!state.busy" placeholder="输入概念说明或代码。代码仅用于书面作答，不会执行。"
          ...
          <div class="flex flex-wrap items-center gap-3">
            <UiButton variant="dark" :disabled="!!state.busy || current.attempts.length >= 3 || !configured" @click="practice.review()">提交作答</UiButton>
          ...
          <section v-if="attempt" ref="feedbackEl" aria-label="答题反馈" class="space-y-3 rounded-xl bg-page-cream p-4">
            ...
            <div v-if="attempt.feedback.strengths.length"><p class="text-caption font-bold">正确要点</p><ul class="mt-1 list-inside list-disc space-y-1 text-body-sm"><li v-for="s in attempt.feedback.strengths" :key="s">{{ s }}</li></ul></div>
            <div v-if="attempt.feedback.gaps.length"><p class="text-caption font-bold">待补充内容</p><ul class="mt-1 list-inside list-disc space-y-1 text-body-sm"><li v-for="gap in attempt.feedback.gaps" :key="gap">{{ gap }}</li></ul></div>
            ...
            <button type="button" class="text-body-sm font-bold underline" @click="help">查找基础课</button>
          </section>
