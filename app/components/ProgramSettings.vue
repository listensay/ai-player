...
<template>
  <section class="mt-5 border-t border-linen pt-4" aria-label="完整学习计划">
    <div class="flex items-center justify-between gap-3">
      ...
      <button v-if="!editing" type="button" class="text-caption font-bold underline" :disabled="!!guide.state.busy" @click="edit">{{ program ? '编辑计划' : '设置计划' }}</button>
    </div>
    ...
    <form v-else class="mt-3 space-y-3 text-body-sm" @submit.prevent="save">
      <div class="grid grid-cols-2 gap-3">
        <label class="block font-bold">总天数<input v-model.number="form.days" type="number" min="1" max="1095" step="1" required class="program-input mt-1" /></label>
        <label class="block font-bold">开始日期<input v-model="form.startDate" type="date" required class="program-input mt-1" /></label>
      </div>
      <fieldset class="rounded-xl border border-linen p-3">
        <legend class="px-1 text-caption font-bold text-stone">每日时间分配（分钟）</legend>
        <div class="grid grid-cols-2 gap-3">
          <label class="block">{{ BUDGET_LABELS.video }}<input v-model.number="form.video" type="number" min="0" max="1440" step="5" required class="program-input mt-1" /></label>
          <label class="block">{{ BUDGET_LABELS.code }}<input v-model.number="form.code" type="number" min="0" max="1440" step="5" required class="program-input mt-1" /></label>
          <label class="block">{{ BUDGET_LABELS.project }}<input v-model.number="form.project" type="number" min="0" max="1440" step="5" required class="program-input mt-1" /></label>
          <label class="block">{{ BUDGET_LABELS.recap }}<input v-model.number="form.recap" type="number" min="0" max="1440" step="5" required class="program-input mt-1" /></label>
        </div>
        <p class="mt-2 text-caption text-stone">合计 {{ formatMinutes(total) }}。各阶段可在实践安排中单独设定分配；看课时间同步为播放器的每日排期。</p>
      </fieldset>
      <div class="grid grid-cols-2 gap-3">
        <label class="block">复盘日间隔（天）<input v-model.number="form.lightEvery" type="number" min="0" max="30" step="1" required class="program-input mt-1" /></label>
        <label class="block">复盘日时长（分钟）<input v-model.number="form.lightMinutes" type="number" min="5" max="1440" step="5" required class="program-input mt-1" /></label>
      </div>
    ...
    <div class="mt-4 rounded-xl bg-page-cream p-3">
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        ...
        <UiButton size="sm" variant="text" :disabled="!!guide.state.busy" @click="fileInput?.click()">导入实践安排</UiButton>
        <input ref="fileInput" type="file" accept=".json,application/json" class="hidden" aria-label="选择实践安排文件" @change="importFile" />
      </div>
...
.program-input { @apply block w-full min-w-0 rounded-xl border border-linen bg-pure-white px-3 py-2 font-normal text-charcoal-ink focus:border-charcoal-ink focus:outline-none; }
</style>
