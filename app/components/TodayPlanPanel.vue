...
const practiceMinutes = computed(() => {
  ...
  return Math.max(0, guide.todayTotalMinutes.value - (guide.todayBudget.value.video || 0))
})
      ...
      <div class="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-page-cream p-4">
        <div>
          ...
          <p class="mt-1 text-caption text-stone">
            {{ guide.program.value ? '已排视频时长' : '任务时长' }} {{ formatStudyDuration(totalSeconds) }} ·
            {{ guide.program.value ? '计划看课' : '计划投入' }} {{ today?.minutes ?? 30 }} 分钟
          </p>
        </div>
