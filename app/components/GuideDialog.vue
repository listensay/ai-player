      ...
      <div class="scroll-soft min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          <section class="pane p-5">
            ...
            <form class="mt-4 space-y-4" @submit.prevent="generate">
              ...
              <label class="flex items-center justify-between gap-3 text-body-sm">{{ state.plan?.program ? '每日看课时间' : '每日学习时间' }}
                <span class="flex items-center gap-2"><VTextField v-model.number="dailyMinutes" aria-label="每天学习分钟数" type="number" min="5" max="1440" step="1" required :disabled="!!state.busy" class="text-center" style="width: 5rem" @change="updateDailyMinutes" />分钟</span>
              </label>
            ...
            <UiButton v-if="!guide.configured.value" variant="text" size="sm" class="mt-3" @click="tab = 'settings'">连接 AI 服务 →</UiButton>
              ...
              <div class="mt-5 border-t border-linen pt-4"><h4 class="text-caption font-bold text-stone">当前学情画像</h4><p class="mt-2 whitespace-pre-wrap break-words text-body-sm leading-relaxed">{{ state.plan.profile }}</p></div>
                <VExpansionPanels v-if="state.plan.messages.length" class="my-3">
                  <VExpansionPanel value="content">
                    <VExpansionPanelTitle>查看导学对话</VExpansionPanelTitle>
                    <VExpansionPanelText>
          ...
          <div v-else class="min-w-0 space-y-5">
            ...
            <section class="pane p-5">
              ...
              <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
                <label class="flex items-center gap-2 text-body-sm"><VCheckbox v-model="state.includeOptional" :disabled="!!state.busy" class="shrink-0" />路线包含选修课</label>
            ...
            <section class="pane p-5">
              ...
              <VTextField :model-value="lessonQuery" type="search" label="搜索路线课节或知识点" placeholder="搜索课节或知识点"
        ...
        <section v-if="tab === 'help'" class="mx-auto max-w-2xl space-y-5">
          ...
          <form ref="questionForm" class="pane p-5" @submit.prevent="findFallback">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3"><UiButton :disabled="!!state.busy || !question.trim()" @click="saveQuestionDraft">存入疑问清单</UiButton><UiButton variant="primary" type="submit" :disabled="!!state.busy || !currentVideo || !state.plan">查找基础课</UiButton></div>
