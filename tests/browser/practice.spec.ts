import { test, expect, titles, plan, courseId, openCourse, configure, generate } from './fixtures'
...
const practice = (page: Page) => page.getByRole('dialog', { name: '课后练习', exact: true })
...
test('小练习：材料不足不请求 AI，作答、显式掌握与草稿支持恢复', async ({ page, desktop }, testInfo) => {
  ...
  await practice(page).getByRole('button', { name: '生成练习', exact: true }).click()
  ...
  await practice(page).getByLabel('作答内容', { exact: true }).fill('用 getMethod 获得方法。')
  await practice(page).getByRole('button', { name: '提交作答', exact: true }).click()
  ...
  await practice(page).getByLabel('作答内容', { exact: true }).fill('改进草稿：获取方法后，向 invoke 传入目标对象。')
  ...
  await enterPractice(page)
  await expect(practice(page).getByLabel('作答内容', { exact: true })).toHaveValue('改进草稿：获取方法后，向 invoke 传入目标对象。')
  await page.reload()
...
test('片段提醒：跳转不触发，连续播放片段结束提醒且可按片段出题', async ({ page, desktop }, testInfo) => {
  desktop.course.duration = 360
  const requests: string[] = []
  await page.route('https://guide.test/v1/chat/completions', route => {
    const prompt = route.request().postDataJSON().messages.at(-1).content as string
    requests.push(prompt)
    return route.fulfill({ json: { choices: [{ message: { content: JSON.stringify(prompt.includes('课后练习') ? question : plan()) } }] } })
  })
  await openCourse(page)
  await desktop.writeCourseFile('02-反射.srt', `1\n00:00:10,000 --> 00:00:20,000\n${material}\n\n2\n00:05:01,000 --> 00:05:10,000\n片段以外不应发送。\n`)
  ...
  await page.getByLabel('今日可用分钟数', { exact: true }).fill('5')
  await page.getByLabel('今日可用分钟数', { exact: true }).press('Tab')
  ...
  await expect(page.locator('[data-fullscreen="true"]').getByLabel('学习片段结束提醒', { exact: true })).toBeVisible()
  expect(await page.locator('video').evaluate((v: HTMLVideoElement) => v.paused)).toBe(false)
  expect((desktop.read('guide', { courseId: courseId(page) }) as any)?.today?.items.some((i: any) => i.done)).toBe(false)
  await page.screenshot({ path: testInfo.outputPath('segment-reminder.png') })
  await reminder.getByRole('button', { name: '开始练习', exact: true }).click()
  await expect.poll(() => desktop.window.fullscreen).toBe(false)
  await expect(practice(page)).toContainText('出题范围：00:00–05:00')
  await practice(page).getByRole('button', { name: '生成练习', exact: true }).click()
  await expect(practice(page).getByRole('heading', { name: question.prompt })).toBeVisible()
...
test('小练习：使用未保存笔记，拒绝伪造依据，取消和服务失败保留已有作答', async ({ page, desktop }) => {
  ...
  await practice(page).getByRole('button', { name: '生成练习', exact: true }).click()
  await expect(practice(page).getByRole('heading', { name: question.prompt })).toBeVisible()
  await practice(page).getByLabel('作答内容', { exact: true }).fill('保留这份答案')
  mode = 'bad'
  await practice(page).getByRole('button', { name: '生成新题', exact: true }).click()
  await expect(practice(page).getByRole('alert')).toContainText('不存在的学习材料')
  await expect(practice(page).getByLabel('作答内容', { exact: true })).toHaveValue('保留这份答案')
  mode = 'error'
  await practice(page).getByRole('button', { name: '提交作答', exact: true }).click()
...
test('片段自然结束可以标记完成；窄屏练习可作答且不溢出', async ({ page, desktop }, testInfo) => {
  await page.route('https://guide.test/v1/chat/completions', route => {
    const body = route.request().postData()!
    return route.fulfill({ json: { choices: [{ message: { content: JSON.stringify(body.includes('课后练习') ? question : plan()) } }] } })
  })
  ...
  await practice(page).getByRole('button', { name: '生成练习', exact: true }).click()
  await expect(practice(page).getByRole('heading', { name: question.prompt })).toBeVisible()
  await practice(page).getByLabel('作答内容', { exact: true }).fill('窄屏也能作答')
