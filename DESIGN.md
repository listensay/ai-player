...
学习首页最大宽度 1440px。桌面主栏依次展示黄色继续学习卡片、按课程折叠的今日安排和课程卡片；右栏为今日投入与紧凑周回顾，小屏自然纵向排列。卡片内边距 20–28px、区域间距 24px，课程列表分页，任务和日期明细局部滚动。

课程筛选、预算与日期输入使用 Vuetify。课程卡片分别展示观看完成、当前阶段和预计视频结束日期，不合并成综合学习分数。周图使用实际投入，选中日期突出边框，并可展开各课程的任务清单；历史计划缺失与课程读取失败需明确显示。

计划调整弹窗使用 `VDialog`，最大宽度 880px、高度 90vh、24px 圆角、桌面内边距 28px、小屏 20px。顶部和底部固定，中间表单与预览滚动，保留浮动标签空间。预览在奶油色阅读面展示今日任务、结束日期对比、未来七天预算及阶段变化；确认前不修改记录，时间缺口阻止应用并提示调整上限或顺延。
